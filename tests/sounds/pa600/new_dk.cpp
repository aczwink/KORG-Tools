...
TEST_SUITE(NewDrumKitSoundTests)
{
	TEST_CASE(Test)
	{
		FileSystem::Path setPath(u8"testdata/sounds/pa600/new_dk.SET");
		Set set(setPath);
		...
		ASSERT_EQUALS(true, soundData.voiceAssignModeFlags.IsSet(VoiceAssignModeFlags::IsDrumKit));
		...
		ASSERT_EQUALS(99, layer1.drumSampleEqualizer.trim);
	}
}