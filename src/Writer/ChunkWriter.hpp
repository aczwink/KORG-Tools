...
public:
	//Constructor
	inline ChunkWriter(SeekableOutputStream& outputStream) : outputStream(outputStream), dataWriter(true, outputStream)
	{
	}
...
protected:
	...
	//Inline
	inline void WriteUnknownChunk(const libKORG::UnknownChunk& chunk)
	{
		this->BeginChunk(chunk.Header().type, chunk.Header().version.major, chunk.Header().version.minor, chunk.Header().flags);
		chunk.Buffer().CreateInputStream()->FlushTo(this->outputStream);
		this->EndChunk();
	}

private:
	//Members
	DynamicArray<uint64> chunkDataOffsets;
